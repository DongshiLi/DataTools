   ...
   public class Stochastic
    {
        ...
        public int smaPeriodCountK;
        public int smaPeriodCountD;

        public Stochastic(List<KeyValuePair<DateTime, Quote>> stock, List<KeyValuePair<DateTime, (decimal, decimal, decimal, decimal)>> ohlc, int close, int rolling, int smak, int smad)
        {
            this.StockDetails = stock;
            this.StockOHLCList = ohlc;
            this.ClosePeriodCount = close;
            this.RollingPeriodCount = rolling;
            this.smaPeriodCountK = smak;
            this,smaPeriodCountD = smad;
        }
        ...
        public int stoTradingRule(int index, decimal OpenDiff, decimal Price, int Position, string newFileName, DateTime StopTime)
        {
            int Side = 0;
            Logger logger = new Logger("start");
            var FASTSTO = QH_IndicatorUtils.FASTSTO(StockOHLCList, RollingPeriodCount, smaPeriodCountK);
            var FASTSTOOSC = QH_IndicatorUtils.FASTSTOOSC(StockOHLCList, RollingPeriodCount, smaPeriodCountK);
            var FULLSTO = QH_IndicatorUtils.FULLSTO(StockOHLCList, RollingPeriodCount, smaPeriodCountK, smaPeriodCountD);
            if ((CompareList(index) >= 1) && (CompareList(index) != CompareList(index - 1)))
            {
                ...
                if (FASTSTOOSC[CompareList(index)] > 0)
                {
                    OpenDiff = (decimal)(FASTSTOOSC[CompareList(index)]);
                ...
                else if (FASTSTOOSC[CompareList(index)] < 0)
                {
                    OpenDiff = (decimal)(FASTSTOOSC[CompareList(index)]);
