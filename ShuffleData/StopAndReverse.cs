   ...
   public class StopAndReverse
    {
        ...
        public int StepPeriodCount;
        public int MaximumStepPeriodCount;

        public StopAndReverse(List<KeyValuePair<DateTime, Quote>> stock, List<KeyValuePair<DateTime, (decimal, decimal, decimal, decimal)>> ohlc, int close, int step, int maximumstep)
        {
            this.StockDetails = stock;
            this.StockOHLCList = ohlc;
            this.ClosePeriodCount = close;
            this.StepPeriodCount = step;
            this.MaximumStepPeriodCount = maximumstep;
        }
        ...
        public int sarTradingRule(int index, decimal OpenDiff, decimal Price, int Position, string newFileName, DateTime StopTime)
        {
            int Side = 0;
            Logger logger = new Logger("start");
            var SAR = QH_IndicatorUtils.SAR(StockOHLCList, StepPeriodCount, MaximumStepPeriodCount);
            ...
            if ((CompareList(index) >= 1) && (CompareList(index) != CompareList(index - 1)))
            {
                ...
                if (StockPrice > SAR[CompareList(index)])
                {
                    OpenDiff = (decimal)(SAR[CompareList(index)]);
                ...
                else if (StockPrice < SAR[CompareList(index)])
                {
                    OpenDiff = (decimal)(SAR[CompareList(index)]);
