   ...
   public class RelativeStrengthIndex
    {
        ...
        public RelativeStrengthIndex(List<KeyValuePair<DateTime, Quote>> stock, List<KeyValuePair<DateTime, (decimal, decimal, decimal, decimal)>> ohlc, int close, int rolling)
        {
        ...
        public int rsiTradingRule(int index, decimal OpenDiff, decimal Price, int Position, string newFileName, DateTime StopTime)
        {
            int Side = 0;
            Logger logger = new Logger("start");
            var RSI = QH_IndicatorUtils.RSI(StockOHLCList, RollingPeriodCount);
            if ((CompareList(index) >= 1) && (CompareList(index) != CompareList(index - 1)))
            {
                ...
                if (RSI[CompareList(index)] > 70)
                {
                    OpenDiff = (decimal)(RSI[CompareList(index)]);
                ...
                else if (RSI[CompareList(index)] < 30)
                {
                    OpenDiff = (decimal)(RSI[CompareList(index)]);
