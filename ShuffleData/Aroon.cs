   ...
   public class Aroon
    {
        ...
        public Aroon(List<KeyValuePair<DateTime, Quote>> stock, List<KeyValuePair<DateTime, (decimal, decimal, decimal, decimal)>> ohlc, int close, int rolling)
        {
        ...
        public int aroonTradingRule(int index, decimal OpenDiff, decimal Price, int Position, string newFileName, DateTime StopTime)
        {
            int Side = 0;
            Logger logger = new Logger("start");
            var Aroon = QH_IndicatorUtils.AROON(StockOHLCList, RollingPeriodCount);
            var AroonOsc = QH_IndicatorUtils.AROONOSC(StockOHLCList, RollingPeriodCount);
            if ((CompareList(index) >= 1) && (CompareList(index) != CompareList(index - 1)))
            {
                ...
                if (Aroon[CompareList(index)].Item1 > 70)
                {
                    OpenDiff = (decimal)(Aroon[CompareList(index)].Item1);
                ...
                else if (Aroon[CompareList(index)].Item2 < 30)
                {
                    OpenDiff = (decimal)(Aroon[CompareList(index)].Item2);
