   ...
   public class AverageTrueRange
    {
        ...
        public AverageTrueRange(List<KeyValuePair<DateTime, Quote>> stock, List<KeyValuePair<DateTime, (decimal, decimal, decimal, decimal)>> ohlc, int close, int shortperiod, int longperiod)
        {
        ...
        public int atrTradingRule(int index, decimal OpenDiff, decimal Price, int Position, string newFileName, DateTime StopTime)
        {
            int Side = 0;
            Logger logger = new Logger("start");
            var shortATR = QH_IndicatorUtils.ATR(StockOHLCList, ShortPeriodCount);
            var longATR = QH_IndicatorUtils.ATR(StockOHLCList, LongPeriodCount);
            if ((CompareList(index) >= 1) && (CompareList(index) != CompareList(index - 1)))
            {
                ...
                if ((shortATR[CompareList(index)] - longATR[CompareList(index)] > 0) && (shortATR[CompareList(index) - 1] - longATR[CompareList(index) - 1] <= 0))
                {
                    OpenDiff = (decimal)(shortATR[CompareList(index)] - longATR[CompareList(index)]);
                ...
                else if ((shortATR[CompareList(index)] - longATR[CompareList(index)] < 0) & (shortATR[CompareList(index) - 1] - longATR[CompareList(index) - 1] >= 0))
                {
                    OpenDiff = (decimal)(shortATR[CompareList(index)] - longATR[CompareList(index)]);
